...
function y = RKorden4SEDO(f1,f2,x,yo,h)
  ...
  y = zeros(n,N+1);   % Matriz de las n variables dependientes que contienen cada una N+1 puntos
                      % La fila representa a la i-esima variable dependiente
  for j=1:n
    y(j,1) = yo(j);   % Se le asigna a la variable j-esima, el valor inicial yo a la primera componente
  endfor
  xo = x(1);          % Valor inicial del intervalo
  for i=2:N+1
    % Calculo los coeficientes ks (relaciones de recurrencia)
    % k es una matriz donde k(i,j) representa el i-esimo valor de k para la j-esima EDO
    % Coeficientes k1s
    ...
    % Aplico la ecuacion de RK de cuarto orden para predecir el valor de la
